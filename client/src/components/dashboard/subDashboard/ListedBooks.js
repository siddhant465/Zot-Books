...
export class ListedBooks extends Component {
  state = {
    searchValue: "",
    bookList: [{}],
    isLoading: true,
  ...
  componentDidMount() {
    axios
      .get(`http://localhost:5000/api/books/getAllBooks`)  //change
      .then((res) => {
  ...
  getListedBookCard = (bookObj) => {
    return (
      <Grid item>
        <ListedBookCard user={this.state.user} {...bookObj} />
      </Grid>
    );
  };
      ...
      return (
        ...
        <Grid className={classes.paper}>
          ...
          <Grid container item spacing={5} justify="flex-start">
            {this.state.bookList.map((bookObj) => this.getListedBookCard(bookObj))}
...
export default withStyles(useStyles)(ListedBooks);
